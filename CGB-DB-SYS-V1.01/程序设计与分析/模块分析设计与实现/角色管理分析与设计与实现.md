...
这两个标签可以使用两种方法将查询到的数据进行凝聚

1. `select`方法

   使用`select`属性将select语句查询到的内容放置查询结果中

   `select`属性的值可以是select语句的id值,也可以是dao接口的方法路径
   ...
   <resultMap id = "RoleMenu">
       ...
       <collection property="menuIds" column="menuId"
   </resultMap>
   ```
